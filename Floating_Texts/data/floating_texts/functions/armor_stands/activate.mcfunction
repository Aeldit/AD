...
execute unless data storage floating_texts:settings ft_settings{auto_show_title: "ON"} run execute unless entity @e[type=armor_stand,tag=sp_selected,limit=1,distance=..2,nbt={CustomNameVisible: 1b}] run data modify entity @e[type=armor_stand,tag=sp_selected,limit=1,distance=..2] CustomNameVisible set value 1b
